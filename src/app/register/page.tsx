import LoginForm, { LoginServerState } from "@/components/forms/login-form";
import RegisterForm, { RegisterPayload, RegisterServerState } from "@/components/forms/register-form";
import { useServerSession } from "@/hooks/useSession";
import { ResponseCookie } from "next/dist/compiled/@edge-runtime/cookies";
import { cookies } from "next/headers";
import { redirect } from "next/navigation";
import { pool } from "../layout";
import { fetchOne } from "@/utils/api";
import { createHash } from "crypto";
import { hashSync } from "bcryptjs";


export default async function Page() {
    const { user } = await useServerSession();

    if (user) return redirect("/");
    async function authorize(state: RegisterServerState, payload: FormData): Promise<RegisterServerState> {
        "use server";

        let body = Object.fromEntries(payload) as unknown as RegisterPayload;



        let captcha = body["cf-turnstile-response"];
        if (!captcha) return {
            ok: false,
            hints: {
                email: "You haven't completed captcha.",
                password: "You haven't completed captcha.",
                username: "You haven't completed captcha.",
            },
            message: "You haven't completed captcha."
        }


        let captchaResponse = await fetch(`https://challenges.cloudflare.com/turnstile/v0/siteverify`, {
            method: "POST",
            body: JSON.stringify({
                "secret": process.env.CAPTCHA_SECRET_KEY,
                "response": captcha
            }),
            headers: {
                'content-type': 'application/json'
            },
            cache: "no-cache"
        }).then(res => res.json()) as { success: boolean };

        if (!captchaResponse.success) return {
            ok: false,
            hints: {
                email: "You haven't completed captcha.",
                password: "You haven't completed captcha.",
                username: "You haven't completed captcha.",
            },
            message: "You haven't completed captcha."
        }

        let user = await fetchOne(`SELECT * FROM "User" WHERE "usernameSafe" = $1 OR "email" = $2`, [body.username.toLowerCase().replaceAll(" ", "_"), body.email]) as { username: string, usernameSafe: string, email: string };

        if (user && body.username.toLowerCase().replaceAll(" ", "_") == user.usernameSafe) return {
            hints: {
                email: "",
                password: "",
                username: "User with this username already exists."
            },
            message: "User with this username already exists.",
            ok: false
        }

        if (user && user.email == body.email) return {
            hints: {
                email: "User with this E-Mail address is already exists.",
                password: "",
                username: ""
            },
            message: "User with this E-Mail is already exists.",
            ok: false
        }

        //Validating username

        if (!/^[a-zA-z0-9\-_ ]{3,15}$/g.test(body.username) || body.username.length < 3 || body.username.length > 15) return {
            hints: {
                username: "Username doesn't match our criteria.",
                email: "",
                password: ""
            },
            message: "Username doesn't match our criteria.",
            ok: false
        }

        if (/^[0-9]*$/g.test(body.username)) return {
            ok: false,
            hints: {
                username: "Username can't should contain at least 1 letter.",
                email: "",
                password: ""
            },
            message: "Username can't should contain at least 1 letter."
        };

        //inserting user
        let hashedPassword = hashSync(createHash("md5").update(body.password).digest("hex"));
        console.log({hashedPassword, md5: createHash("md5").update(body.password).digest("hex")});

        let createdUser = await pool.query(`INSERT INTO "User"("username", "usernameSafe", "password", "email", "flags", "permissions", "createdAt") VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING "id"`, [
            body.username, body.username.replaceAll(" ", "_"), hashedPassword, body.email, 32, 8, new Date()]).then(r => r.rows[0]) as { id: number };

        console.log(createdUser);
        if (!createdUser.id) return {
            ok: false,
            message: "",
            hints: {
                username: "Something failed while inserting users, contact administrator.",
                email: "",
                password: ""
            }
        }

        await pool.query(`INSERT INTO "UserStats"("userId") VALUES ($1)`, [createdUser.id]);

        console.log(JSON.stringify({
            client_id: process.env.LISEK_OAUTH2_CLIENT_ID,
            client_secret: process.env.LISEK_OAUTH2_CLIENT_SECRET,
            grant_type: "password",
            scope: "*",
            username: body.username,
            password: body.password
        }));
        let response = await fetch(`https://${process.env.NODE_ENV == "development" ? "dev." : ""}lisek.world/oauth/token`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                client_id: process.env.LISEK_OAUTH2_CLIENT_ID,
                client_secret: process.env.LISEK_OAUTH2_CLIENT_SECRET,
                grant_type: "password",
                scope: "*",
                username: body.username,
                password: body.password
            }),
            cache: "no-cache"
        }).then(res => res.json());

        if (response.error && !["username", "password"].includes(response.hint)) return {
            ok: false,
            hints: {
                username: response.message,
                password: response.message,
                email: ""
            },
            message: response.message
        }

        if (response.error) return {
            ok: false,
            hints: {
                username: response.hint == "username" ? response.message : "",
                password: response.hint == "password" ? response.message : "",
                email: response.hint == "username" ? response.message : ""
            },
            message: response.message
        };

        const { access_token, token_type, expires_in, refresh_token } = response;

        let cookie = cookies();
        let options = {
            expires: new Date(Date.now() + (1000 * 60 * 60 * 24 * 21)),
        } as ResponseCookie;
        cookie.set("auth.access_token", access_token, options);
        cookie.set("auth.token_type", token_type, options);
        cookie.set("auth.refresh_token", refresh_token, options);
        cookie.set("auth.expires_in", (Date.now() / 1000) + expires_in, options);

        let origin = cookie.get("auth.origin")?.value ?? "/";
        return redirect(origin);

    }


    return (<div className="w-full min-h-screen flex justify-center items-center -mt-[10%] animate-fade-up animate-once animate-duration-[300ms] animate-ease-linear">
        <div className="bg-background-900/20 w-full sm:w-[30%] min-h-10 flex flex-col gap-5 justify-center items-center rounded-xl">
            <div className="text-3xl bg-background-950/50 w-full min-h-10 py-10 flex justify-center">
                Login
            </div>
            <RegisterForm action={authorize} />
        </div>
    </div>)
}